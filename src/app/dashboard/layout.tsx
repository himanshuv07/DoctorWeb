...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen overflow-hidden bg-[#0d0f1e] md:h-screen">
      
      {/* ── Sidebar ── */}
      <aside className={cn(
        'fixed inset-y-0 left-0 z-50 flex flex-col bg-[#0f0f14] border-r border-white/[0.06] transition-all duration-300 ease-in-out shrink-0 shadow-2xl md:relative md:z-auto',
        {
          'w-[68px]': collapsed || isMobile,
          'w-[230px]': !collapsed && !isMobile,
          'w-72': !collapsed && isMobile && showMobileMenu,
          'translate-x-[-100%] md:translate-x-0': isMobile && !showMobileMenu
        }
      )}>
        ...
        {/* Logo */}
        <div className={cn('flex items-center gap-2.5 px-4 py-5 shrink-0', 
          (collapsed || (isMobile && !showMobileMenu)) && 'justify-center px-0')}>
          ...
          <span className={cn("text-lg font-bold tracking-[0.1em] text-white origin-left transition-all duration-300",
            collapsed || (isMobile && !showMobileMenu) ? 'scale-0 opacity-0 w-0' : 'scale-100 opacity-100 w-auto'
          )}>
        ...
        <nav className="flex-1 overflow-y-auto overflow-x-hidden py-4 px-2 space-y-0.5 max-h-[calc(100vh-200px)]">
          {navItems.map(({ label, href, icon: Icon }) => {
            ...
            return (
              <Tooltip key={href} delayDuration={0}>
                <TooltipTrigger asChild>
                  <button
                    ...
                    <Icon className={cn(
                      ...
                      (collapsed || (isMobile && !showMobileMenu)) ? 'w-5 h-5' : 'w-4 h-4',
                ...
                {(collapsed || (isMobile && !showMobileMenu)) && (
        ...
        <div className={cn('p-3 shrink-0', (collapsed || (isMobile && !showMobileMenu)) && 'flex justify-center')}>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className={cn(
                'w-full flex items-center gap-2.5 p-2 rounded-xl hover:bg-white/[0.05] transition-all',
                (collapsed || (isMobile && !showMobileMenu)) && 'w-auto justify-center'
              )}>
                ...
                <div className={cn("flex-1 text-left overflow-hidden transition-all duration-300",
                  collapsed || (isMobile && !showMobileMenu) ? 'scale-0 opacity-0 w-0' : 'scale-100 opacity-100 w-auto'
                )}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
        {/* Responsive Topbar */}
        <header className="shrink-0 h-14 bg-[#0f0f14] border-b border-white/[0.06] flex items-center justify-between px-4 sm:px-6 gap-3 sm:gap-4 relative z-30">
          
          {/* Mobile Menu Button */}
          {isMobile && (
            <button
              onClick={() => setShowMobileMenu(!showMobileMenu)}
              className="p-2 rounded-xl bg-white/[0.05] hover:bg-white/[0.1] border border-white/[0.08] text-white/60 hover:text-white transition-all md:hidden"
            >
          ...
          <div className="flex-1 max-w-xs sm:max-w-md lg:w-72">
            <div className="flex items-center bg-white/[0.05] border border-white/[0.08] rounded-xl
                            px-3.5 py-2 focus-within:border-violet-500/40 focus-within:bg-white/[0.07] transition-all">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button className="relative w-8 h-8 rounded-xl bg-white/[0.05] hover:bg-white/[0.09] border border-white/[0.08]
                               flex items-center justify-center transition-colors p-1.5">
              <Bell className="w-4 h-4 text-white/50" />
              <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-violet-500" />
            </button>
            <Badge className="bg-violet-500/15 text-violet-300 border border-violet-500/30 text-[11px] font-medium px-2.5 hidden sm:inline-flex">
              Admin
            </Badge>
        ...
        {/* Page area */}
        <main className="flex-1 overflow-y-auto bg-[#0d0f1e] w-full">
          {children}
        </main>
      </div>
    </div>
