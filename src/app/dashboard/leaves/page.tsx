...
function StatusBadge({ status }: { status: "enabled" | "disabled" }) {
    return (
        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] sm:text-[11px] font-semibold border ${status === "enabled"
...
export default function LeavesPage() {
    ...
    const filtered = useMemo(() => {
        ...
        return leaves.filter((l) => {
            const doctorName = `${l.doctor?.fname || ""} ${l.doctor?.lname || ""}`.toLowerCase();

            const matchSearch =
                !q ||
                doctorName.includes(q) ||
                l.leave_startDate?.toLowerCase()?.includes(q) ||
                l.leave_endDate?.toLowerCase()?.includes(q) ||
                        ...
                        <div className="overflow-y-auto flex-1 px-4 sm:px-5 py-4 sm:py-5 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full rounded-lg sm:rounded-xl border bg-[#141a2f] px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-white outline-none transition-all [color-scheme:dark] ${fieldErrors.leave_startDate
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full rounded-lg sm:rounded-xl border bg-[#141a2f] px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-white outline-none transition-all [color-scheme:dark] ${fieldErrors.leave_endDate
                                            ? "border-red-500/60 focus:border-red-500 focus:ring-2 focus:ring-red-500/15"
                                            : "border-white/10 focus:border-violet-500/60 focus:ring-2 focus:ring-violet-500/15"
                                            }`}
                                    />
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={3}
                                    className={`w-full rounded-lg sm:rounded-xl border bg-[#141a2f] px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm text-white outline-none placeholder:text-slate-600 transition-all resize-none ${fieldErrors.remark
                                        ? "border-red-500/60 focus:border-red-500 focus:ring-2 focus:ring-red-500/15"
                                        : "border-white/10 focus:border-violet-500/60 focus:ring-2 focus:ring-violet-500/15"
                                        }`}
                                />
