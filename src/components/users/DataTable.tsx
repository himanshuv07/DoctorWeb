    ...
    return (
        <>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
                <button
                    ...
                    className="inline-flex items-center gap-2 px-5 py-2.5 bg-violet-600 hover:bg-violet-500
                     active:scale-[0.98] text-white text-sm font-semibold rounded-xl transition-all
                ...
                <div className="relative">
                    ...
                    <input ref={searchRef} type="text" placeholder="Search users..." value={search}
                        ...
                        className="pl-9 pr-8 py-2.5 w-60 bg-white/[0.05] border border-white/[0.08] rounded-xl text-sm
            ...
            <div className="flex items-center gap-1.5 mb-5 flex-wrap">
                <span className="text-xs text-slate-500 font-medium mr-1">Role:</span>
            ...
            <div className="flex items-center gap-2 mb-4 text-xs text-slate-500">
                ...
                <select value={pageSize} onChange={e => onPageSize(Number(e.target.value))}
                    className="bg-white/[0.05] border border-white/[0.08] rounded-lg px-2 py-1 text-slate-300
                     outline-none focus:ring-1 focus:ring-violet-500/40 cursor-pointer [&>option]:bg-[#13152a]">
                        ...
                        <tr className="bg-white/[0.04] border-b border-white/[0.06]">
                            {COLS.map(({ key, label }) => (
                                <th key={key} onClick={() => handleSort(key)}
                                    className="px-4 py-3 text-left text-[11px] font-bold text-slate-400 uppercase
                            ...
                            <th className="px-4 py-3 text-center text-[11px] font-bold text-slate-400 uppercase tracking-wider">Action</th>
                        </tr>
                    </thead>
                        ...
                        {paginated.length === 0 ? (
                            ...
                            <tr key={user._id ?? i} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors">
                                ...
                                <td className="px-4 py-3.5">
                                    ...
                                    flex items-center justify-center text-[11px] font-bold text-violet-400 shrink-0">
                                            {user.displayFirst?.charAt(0)?.toUpperCase() ?? "?"}
                                        </div>
                                        <span className="text-slate-200 font-medium">{user.displayFirst}</span>
                                    </div>
                                </td>

                                <td className="px-4 py-3.5 text-slate-300">{user.displayLast}</td>

                                <td className="px-4 py-3.5">
                                    <a href={`tel:${user.phone}`} className="text-slate-400 hover:text-violet-400 transition-colors font-mono text-xs">
                                        {user.phone}
                                    </a>
                                </td>

                                <td className="px-4 py-3.5">
                                    <a href={`mailto:${user.email}`} className="text-slate-400 hover:text-violet-400 transition-colors text-sm">
                                        {user.email}
                                    </a>
                                </td>

                                <td className="px-4 py-3.5 whitespace-nowrap">
                                    <span className={`text-sm mr-1.5 ${user.gender?.toLowerCase() === "female" ? "text-pink-400" : "text-blue-400"}`}>
                                        {GENDER_ICON[user.gender?.toLowerCase()] ?? ""}
                                    </span>
                                    <span className="text-slate-400 capitalize text-xs">{user.gender}</span>
                                </td>

                                <td className="px-4 py-3.5">
                                    <span className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold capitalize border ${ROLE_BADGE[user.role] ?? "bg-slate-500/10 text-slate-400 border-slate-500/20"
                                        }`}>
                                        {user.role}
                                    </span>
                                </td>
                                    ...
                                    <div className="flex flex-wrap gap-1 max-w-[220px]">
                                        {user.specialities.length > 0
                                            ? user.specialities.map((s: string) => (
                                                <span key={s}
                                                    className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20
                                       text-emerald-400 text-[11px] rounded-md font-medium whitespace-nowrap">
                                                    {s}
                                                </span>
                                            ))
                                ...
                                <td className="px-4 py-3.5 whitespace-nowrap">
                                    <span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold ${user.isActive ? "text-emerald-400" : "text-slate-500"
                                        }`}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-5 pt-4 border-t border-white/[0.06]">
                ...
                <div className="flex items-center gap-1">
                    <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                        className="px-3 py-1.5 text-xs font-medium rounded-lg border border-white/[0.08] text-slate-400
                    ...
                    {pageButtons.map((p, i) =>
                        p === "..."
                            ...
                            : <button key={p} onClick={() => setPage(p as number)}
                                className={`w-8 h-8 text-xs font-medium rounded-lg transition-all ${page === p ? "bg-violet-600 text-white shadow-md shadow-violet-900/40"
                ...
                <div className="flex flex-col items-center gap-4 text-center">
                    ...
                    <div className="flex gap-2.5 w-full">
                        <button onClick={onCancel}
                            className="flex-1 py-2.5 text-sm font-medium rounded-xl border border-white/10
                         text-slate-300 hover:bg-white/5 transition-colors">Cancel</button>
                        <button onClick={onConfirm}
                            className="flex-1 py-2.5 text-sm font-semibold rounded-xl bg-red-600 hover:bg-red-500
                         text-white transition-colors">Delete</button>
                    </div>
                </div>
            </div>
        </div>
